$.extend(APP.anagrafica_ss,
{
	...
	start: function(el, titolo, section, div)
	{
		...
		else
		{
			this.processSectionsProperty({
				...
				'loadCallback': function(url, res){
					var obj = {};
					if (APP.utils.isset(res))
					{
						obj = {
	...
	showTable: function(url, params) // tableContainer, section, items_per_page, window, oldWindow, onSelectRow
	{
		var that = this;
		...
		//if (!APP.utils.isset(this.sections[section].values))
			//return;
		var iDisplayLength = APP.utils.isset(params.items_per_page)? params.items_per_page : APP.config.default_iDisplayLength;
		...
		tableDiv.find("table.datatable").dataTable({
			...
			{
				var iStart = parseInt(aoData[APP.utils.getIndexFromField(aoData, "name", "iDisplayStart")].value);
				var iLength = parseInt(aoData[APP.utils.getIndexFromField(aoData, "name", "iDisplayLength")].value);
				var ordering = "";
				var iSortingCols = parseInt(aoData[APP.utils.getIndexFromField(aoData, "name", "iSortingCols")].value);
				for (var ite = 0; ite < iSortingCols; ite++)
				{
					var colIndex = parseInt(aoData[APP.utils.getIndexFromField(aoData, "name", "iSortCol_"+ite)].value);
					var colDir = aoData[APP.utils.getIndexFromField(aoData, "name", "sSortDir_"+ite)].value;
					ordering += cols[colIndex].name+":"+colDir+",";
					if (ite === (iSortingCols-1))
						ordering = ordering.substr(0, ordering.length-1);
				}
				
				var concatenator = (sSource.indexOf("?") >= 0)? "&" : "?";
				
				oSettings.jqXHR = $.ajax( {
					"dataType": 'json',
					"type": "GET",
					"url": sSource+concatenator+"page="+(parseInt(iStart/iLength)+1)+"&items_per_page="+iLength+"&ordering="+ordering,
					"data": {},
					"success": function(srvObj, label, xhr)
					{
						var aaData = [];
						$.each(srvObj.data.items, function(i,v)
						{
							var searchValue = v.id;
							/*
							if ($.type(that.sections[section].values[0].id) == "string")
								searchValue = ""+searchValue;
							else
							{
								if ($.type(that.sections[section].values[0].id) == "number")
									searchValue = parseInt(searchValue);
							}
							*/
							if (!APP.utils.isset(that.sections[section].values))
								that.sections[section].values = [];
							that.sections[section].values.push(v);
							...
							$.each(cols, function(j, k)
							{
								...
								if (APP.utils.isset(k.foreign_key))
								{
									...
									item[k.name] = APP.utils.displayData(str, k);
									return true;
								}
								...
								item[k.name] = APP.utils.displayData(v[k.name], k);
							});
						...
						$.extend(aoData, {
							"sEcho": ++that.sEcho,
							"iTotalRecords": APP.utils.isset(srvObj.data.tot_items)? srvObj.data.tot_items : srvObj.data.items.length,
							"iTotalDisplayRecords": APP.utils.isset(srvObj.data.tot_items)? srvObj.data.tot_items : srvObj.data.items.length,
							"aaData": aaData,
						});
			...
			"fnRowCallback": function( nRow, aData, iDisplayIndex )
			{
				...
				var objIndex = APP.utils.getIndexFromField(that.sections[section].values, "id", searchValue);
				if (objIndex > -1)
				{
					tr.data(that.sections[section].values[objIndex]);
					tr.click(function(){ 
	...
	createFormTemplate: function(identifier, dataObject, sectionTarget, sectionLabel, buttons)
	{
		...
		var displayInputs = function(k, parNode)
		{
			$.each(k.fields, function(j1, k1)
			{
				...
				var spanValues = (APP.utils.isset(position) && position === "blockColForm")? [2, 9, 1] : [4, 7, 1];
