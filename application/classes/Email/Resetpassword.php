...
class Email_Resetpassword {
    ...
    public function buildBodyMail()
    {
        ...
        $bodyMail = View::factory('email/resetpassword');
        $bodyMail->global_data = $config;
        $bodyMail->user = $this->user; 
        $this->_bodyMail =$bodyMail; 
        $this->_subject = $config['pre_email_subject'].__('Reset password'); //Segnalazione correttamente rigistrata sul portale
